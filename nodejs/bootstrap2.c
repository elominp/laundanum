...
typedef int (*stage2_main)(napi_env env, napi_value this, napi_value eval, napi_value require, int argc, char ** argv);
...
static napi_value stage2(napi_env env, napi_callback_info info) {
  ...
  printf("Calling C++\n");
  first_run = false;
  cb(env, argv[2], argv[3], argv[4], nb_args, new_args);
  } else {
    cb(env, argv[2], argv[3], argv[4], 0, NULL);
  }
  /*for (size_t i = 0; i < nb_args; i++) {
    free(new_args[i]);
  }*/
